...
if(isset($_POST['send']))
{
  ...
  $q="INSERT INTO service_provider(lo_id,image,companyname,phone,website,address)VALUES($lo_id,'$image','$companyname','$phone','$website','$address')";
  // var_dump($q);
  // exit();
...
$s="SELECT * FROM service_provider INNER JOIN registration on service_provider.lo_id=registration.lo_id WHERE service_provider.lo_id=$lo_id";
		if(!$stmt=mysqli_query($con,$s))
		{
			die("prepare statement error1");
		}
...
if(isset($_POST['edit']))
{
  ...
  $address=$_POST['address'];
   // $email=$_POST['email'];
  $qu="UPDATE service_provider SET companyname='$companyname',website='$website',phone='$phone',address='$address'";

 $result=$con->query($qu);
  if($result)
  {
     // echo "<script>window.location.replace('astrologerspace.php');</script>";
    header("Location:serviceproviderspace.php");
  }
}
